  ...
  switch (workflowState) {
    ...
    case "GENERATE_SCRIPT": {
      ...
      await startChatGPTJob(finalPrompt, {
        activeJobStage: isMultiTurnScript ? "script_outline" : "script",
      }, true);
      break;
    }
...
async function handleResponseComplete(message) {
  ...
  await handleSheetResponse(state, responseText);
}
...
async function handleSheetResponse(state, responseText) {
  if (state.activeJobStage === "script") {
    await setStorage({
  ...
  if (state.activeJobStage === "script_outline") {
    const outlineSectionCount = detectScriptOutlineSectionCount(responseText);
    const patch = {
      scriptOutline: responseText,
      sheetWorkflowState: "GENERATE_SCRIPT_SECTION",
