...
test("cleanScriptParagraph strips ChatGPT continue metadata tail", () => {
  const gas = loadAppsScript();
  const paragraph = [
    "The candle guttered once, then steadied as Mara listened to the rain.",
    "Begins with: The candle guttered once",
    "[Word count: 1186] >>> Awaiting \u201cCONTINUE\u201d"
  ].join("\n");

  assert.equal(
    gas.cleanScriptParagraph(paragraph),
    "The candle guttered once, then steadied as Mara listened to the rain."
  );
  assert.equal(gas.isScriptMetadataLine("[Word count: 1186] >>> Awaiting \u201cCONTINUE\u201d"), true);
});
