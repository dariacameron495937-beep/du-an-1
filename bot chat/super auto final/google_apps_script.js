...
function isScriptMetadataLine(line) {
  ...
  const patterns = [
    ...
    /^(begins?|starts?)\s+with\s*[:.\-\)]\s*.{0,160}$/i,
    /^(approx(?:imate|imately)?\s*)?(word\s*count|words?)\s*[:.\-\)]?\s*(about\s*)?\d[\d,.\s]*(?:\s*words?)?\.?$/,
    /^(approx(?:imate|imately)?\s*)?(word\s*count|words?)\s*[:.\-\)]?\s*(about\s*)?\d[\d,.\s]*(?:\s*words?)?\s*\]?\s*(?:>{2,}|[-:|])\s*(awaiting|waiting\s+for|ready\s+for|standing\s+by\s+for)\s+["'\u201c\u201d]?(continue|viet tiep|tiep tuc)["'\u201c\u201d]?\.?$/i,
